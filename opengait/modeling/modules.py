...
class SpatialPoolingPyramid(nn.Module):
    ...
    def __init__(self, pooling_func):
        super(SpatialPoolingPyramid, self).__init__()
    ...
    def forward(self, seqs,):
        ...
        seqs = seqs.reshape(n,c,s,-1)
        #seqs = torch.mean(seqs,dim=-1)+torch.max(seqs,dim=-1)[0]
        seqs = torch.max(seqs,dim=-1)[0]
        return seqs

class HorizentalTemporalPoolingPyramid():
    """
        Horizontal Pyramid Matching for Person Re-identification
        Arxiv: https://arxiv.org/abs/1804.05275
        Github: https://github.com/SHI-Labs/Horizontal-Pyramid-Matching
    ...
    def __init__(self, max_num=[10]):
        self.max_num = max_num[0]
    def __call__(self, x):
        """
            x  : [n, c, s]
        """
        n,c,s = x.size()
        num = int(s/self.max_num)
        features = []
        for i in range(self.max_num):
            xi = torch.max(x[:,:,i::int(self.max_num)], dim=2)[0]
            features.append(xi.unsqueeze(2))
        return torch.cat(features, -1)
...
class SetBlockWrapperAtt(nn.Module):
    def __init__(self, forward_block):
        super(SetBlockWrapperAtt, self).__init__()
        self.forward_block = forward_block

    def forward(self, x, att, *args, **kwargs):
        """
            In  x: [n, c_in, s, h_in, w_in]
            Out x: [n, c_out, s, h_out, w_out]
        """
        ...
        x,_ = self.forward_block(x.transpose(
            1, 2).reshape(-1, c, h, w), att, s, *args, **kwargs)
        output_size = x.size()
