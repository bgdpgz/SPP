...
class BaseModel(MetaModel, nn.Module):
    ...
    def _load_ckpt(self, save_name):
        ...
        for i in model_state_dict:
            if i not in ["loss_aggregator.losses.triplet_fq.module.fc.weight","loss_aggregator.losses.triplet_fq.module.fc.bias",'loss_aggregator.losses.softmax_Ada.t',"loss_aggregator.losses.softmax_Ada.batch_mean","loss_aggregator.losses.softmax_Ada.batch_std"]: #and 'BNNecks' not in i:
        ...
        if self.training:
            ...
            if not self.engine_cfg["scheduler_reset"] and 'scheduler' in checkpoint:
                ...
                #self.scheduler.milestones = Counter({80000:1, 160000:1, 200000:1})
                # print(self.scheduler.milestones)
            else:
                self.msg_mgr.log_warning(
