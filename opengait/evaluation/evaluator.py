...
def evaluate_Gait3D(data, dataset, metric='euc'):
    ...
    for id, ty, sq in zip(labels, cams, time_seqs):
        if '-'.join([id, ty, sq]) in probe_sets:
            ...
            probe_ls.append('-'.join([id, ty, sq]))
        else:
            probe_mask.append(False)
            gallery_ls.append('-'.join([id, ty, sq]))
...
def identification_BUAADUKEGAIT(data, dataset, name='', metric='euc'):
    msg_mgr = get_msg_mgr()
    feature, label, seq_type, view = data['embeddings'], data['labels'], data['types'], data['views']
    view = [i.split('_')[0] for i in view]

    # print ('feature',feature.shape) #(15, 14, 256)
    # print ('label',label) # ['0005' '0005' '0005' '0019' '0019' '0019' '0019' '0021' '0021' '0021' '0021' '0023' '0023' '0023' '0023']
    # print ('seq_type',seq_type) #['00', '00', '01', '00', '00', '00', '01', '00', '00', '00', '01', '00', '00', '00', '01']
    # print ('view',view) # ['cam5_51784_52494', 'cam6_30886_31726', 'cam2_46867_47582', 'cam1_51233_51978', 'cam5_60506_61211', 'cam6_39590_40430', 'cam2_55596_56291', 'cam1_51740_52515', 'cam7_34896_35441', 'cam8_8516_9111', 'cam6_30258_31073', 'cam1_52085_52740', 'cam2_56227_57037', 'cam4_31384_32279', 'cam3_33774_34329']
    ...
    # print ('view_list',view_list) # ['cam1', 'cam2', 'cam3', 'cam4', 'cam5', 'cam6', 'cam7', 'cam8']
    # print ('view_num',view_num) # 8

    probe_seq_dict = {'BUAADUKEGAIT': ['01']}
    gallery_seq_dict = {'BUAADUKEGAIT': ['00']}
    ...
    # multi-cam
    acc = np.zeros([len(probe_seq_dict[dataset]), num_rank]) - 1.

    gseq_mask = np.isin(seq_type, gallery_seq_dict[dataset][0])
    gallery_x = feature[gseq_mask, :]
    gallery_y = label[gseq_mask]
    pseq_mask = np.isin(seq_type, probe_seq_dict[dataset][0])
    probe_x = feature[pseq_mask, :]
    probe_y = label[pseq_mask]
    ...
    msg_mgr.log_info('===Rank-1 (Multi-camera cases)===')
    msg_mgr.log_info('NM: %.3f ' % (np.mean(acc[0])))
    msg_mgr.log_info(acc)

    result_dict["scalar/test_accuracy/NM"] = acc[0]
    return result_dict
